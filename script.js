...
async function setupPeople() {
  for (let i = 0; i < peopleData.length; i++) {
    ...
    try {
      ...
      console.error(
        `Failed to load image for person ${i}: ${imagePath}`,
        error
      );
    }
  }
...
// Add Stripe Toggle Button
const stripeBtn = document.createElement("button");
stripeBtn.textContent = "Toggle Stripes";
stripeBtn.style.position = "absolute";
stripeBtn.style.top = "60px";
stripeBtn.style.left = "40px";
stripeBtn.style.zIndex = 1002;
stripeBtn.style.fontSize = "1.2em";
stripeBtn.style.padding = "8px 20px";
stripeBtn.style.borderRadius = "8px";
stripeBtn.style.border = "none";
stripeBtn.style.background = "#222";
stripeBtn.style.color = "#fff";
stripeBtn.style.cursor = "pointer";
stripeBtn.style.boxShadow = "0 2px 8px rgba(0,0,0,0.2)";
document.body.appendChild(stripeBtn);
let stripesEnabled = false;
let stripeOffset = 0;
toggleStripes()

// Animate the stripes by updating the offset on each frame
viewer.scene.preRender.addEventListener(function () {
  if (stripesEnabled && viewer.scene.globe.material && viewer.scene.globe.material.uniforms) {
    stripeOffset += 0.002; // Adjust speed as desired
    if (stripeOffset > 1) stripeOffset -= 1;
    viewer.scene.globe.material.uniforms.offset = stripeOffset;
  }
});
...
function toggleStripes() {
  ...
  if (stripesEnabled) {
    viewer.scene.globe.material = Cesium.Material.fromType("Stripe");
    viewer.scene.globe.material.uniforms.oddColor = Cesium.Color.BLACK.withAlpha(0.2);
    viewer.scene.globe.material.uniforms.evenColor = Cesium.Color.CYAN.withAlpha(0.05);
    viewer.scene.globe.material.uniforms.repeat = 40.0;
    viewer.scene.globe.material.uniforms.offset = stripeOffset;
  } else {
    viewer.scene.globe.material = undefined; // disables custom material, shows default imagery
  }
}
...
// Set default zoom to be closer
viewer.scene.camera.setView({
  destination: Cesium.Cartesian3.fromDegrees(0, 20, 6.5e6),
});

// Restrict zoom in/out
viewer.scene.screenSpaceCameraController.minimumZoomDistance = 2.0e6;
viewer.scene.screenSpaceCameraController.maximumZoomDistance = 3.5e7;


viewer.scene.primitives.add(
  new Cesium.ParticleSystem({
    image: "public/map-pin.png",
    startColor: Cesium.Color.CYAN.withAlpha(0.2),
    ...
    emissionRate: 100.0,
    lifetime: 16.0,
    emitter: new Cesium.SphereEmitter(10000000.0),
    modelMatrix: Cesium.Matrix4.fromTranslation(
      Cesium.Cartesian3.fromDegrees(0, 0, -5000000)
    ),
  })
);
...
function showPersonUI(name, time) {
  ...
  uiBar.innerHTML = `
    <div style="font-size:0.5em;">JaneaGuesser</div>
    <div style="display:flex;align-items:center;justify-content:center;gap:16px;">
      ...
      <b style="font-size: 1.0rem">${name}</b>
...
function getHotColdColor(dist) {
  if (dist < 300000) return Cesium.Color.RED;
  if (dist < 600000) return Cesium.Color.ORANGE;
  if (dist < 1000000) return Cesium.Color.YELLOW;
  if (dist < 2000000) return Cesium.Color.CYAN;
...
function placeFlagAt(position, color = Cesium.Color.WHITE) {
  ...
  const flagPosition = Cesium.Cartesian3.add(
    position,
    offset,
    new Cesium.Cartesian3()
  );
